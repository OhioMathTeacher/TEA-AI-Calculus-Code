...
\title{AI-Enhanced Learning in Calculus Education\thanks{Analysis scripts, protocols, and reproducibility documentation are publicly available at \url{https://github.com/OhioMathTeacher/TEA-AI-Calculus-Code}. GenAI tools (OpenAI GPT-5.1 and Anthropic Claude Sonnet 4.5, accessed via VS Code with GitHub Copilot) made substantial contributions to this research through automated transcript analysis, coding workflows, and qualitative data processing. Their methodological role is detailed in Section~\ref{sec:method}.}}
\author{%
...
Building on prior proof-of-concept work demonstrating that AI agents can successfully engage calculus students in inquiry-based explorations within historical contexts (Edwards et al., 2024), we designed a Taylor series module delivered through a genAI interface, positioning the AI as an interactive tutor offering historical narratives, visualizations, and guided problem-solving to encourage iterative reflection.
...
\item\textbf{Primitive Doing}: Initial interactions with physical objects, figures, graphics, or symbols. For calculus students, this could be graphing series or manipulating terms to see patterns.
...
\noindent\textit{How AI Tools Support Each Level of the Pirie-Kieren Model.}
...
\begin{enumerate}[itemsep=0.1em]
\item\textbf{GenAI for Image Making and Having}: GenAI tools illustrate complex series through visualization prompts or dynamic content generation. GenAI also generate examples of sequences and series, encouraging students to create mental models.

\item\textbf{Property Noticing through Problem Posing}: GenAI present curated examples and guide students toward noticing patterns and properties, such as monotonicity or boundedness, critical in identifying convergence.
...
\item\textbf{Inventising and Recursive Inquiry}: GenAI prompt students to generate novel problems or explore edge cases, stimulating deeper recursive thinking. Students can explore hypothetical series or test conjectures interactively, enhancing their ability to think inventively.

\item\textbf{Folding Back Enhanced by GenAI}: GenAI assist students in explicitly revisiting earlier stages, pinpointing misconceptions, or clarifying confusion. By responding adaptively, AI helps students effectively move between different levels of understanding.
\end{enumerate}
...
These cases were chosen to illustrate the diversity of student approaches, agentic moves, and mathematical sophistication achieved during AI-mediated sessions. Across the 30 anchor cases, student talk percentages ranged from 13.3\% to 100.0\%, reflecting a broad spectrum of participation patterns. Regardless of talk share, all cases showed evidence of significant mathematical reasoning, with students reaching at least the Formalizing'' or Observing'' layers, and several attaining ``Structuring'' (the penultimate PK level). The number of observed recursive movements ranged from two to three per transcript, suggesting that the genAI partner frequently prompted cycles of reflection, error-checking, and conceptual reorganization.

The ``Notable Feature(s)'' column highlights key agentic moments—instances where students made creative leaps, adapted their approach, or critically reflected on the limits of formal models.
...
\caption{Anchor Cases with Lowest Student Talk Percentages (Bottom 10).}
\label{tab:2a}
\renewcommand{\arraystretch}{1.1}
...
P12-GX-S6   &  763 &  147 &  910 & 83.8 & 16.2 & Restates AI ideas \\
\bottomrule
\end{tabularx}
\end{threeparttable}
\end{subtable}
...
\textbf{ID} & \textbf{AI Words} & \textbf{Student Words} & \textbf{Total Words} & \textbf{\% AI} & \textbf{\% Student} & \textbf{Notes} \\
\midrule
P76-GX-S6   &  761 & 3228 & 3989 & 19.1 & 80.9 & Technical depth \\
P82-G13-S6  & 1007 & 1518 & 2525 & 39.9 & 60.1 & Student-driven \\
P105-G10-S5 & 1519 & 1042 & 2561 & 40.7 & 59.3 & Dense reasoning \\
P109-G14-S4 & 1095 &  796 & 1891 & 42.1 & 57.9 & Extended thinking \\
P106-GX-S6  & 1166 & 1440 & 2606 & 44.8 & 55.2 & High volume \\
...
\begin{threeparttable}
\vspace{0.8em}
\caption{Noteworthy Anchor Cases with Mid-Range Student Talk Percentages.}
\label{tab:2c}
\renewcommand{\arraystretch}{1.1}
\setlength{\tabcolsep}{3.5pt}
\begin{tabularx}{\linewidth}{
...
P123-GX-S6  & 1326 &  673 & 1999 & 66.3 & 33.7 & Sustained inquiry \\
P90-GX-S6   & 1672 &  781 & 2453 & 68.2 & 31.8 & Strategic thinking \\
P54-GX-S6   & 1283 &  540 & 1823 & 70.4 & 29.6 & Focused reasoning \\
...
Analysis of the thirty anchor cases using the Pirie--Kieren Work Analysis Protocol (PK-WAP) revealed substantial depth of mathematical engagement across all student participation levels. Of the thirty complete analyses, 77\% (23/30) reached \textbf{Inventising}---the highest level of the Pirie--Kieren model, characterized by creating new mathematical concepts or adapting existing frameworks to novel contexts. \emph{It is important to note that reaching Inventising reflects coded transcript evidence of sophisticated reasoning within the dialogue, not a claim about durable cognitive achievement or transfer beyond the activity.} This finding held consistently across engagement categories: 80\% of low-talk cases (bottom 10), 70\% of high-talk cases (top 10), and 80\% of noteworthy mid-range cases reached this sophisticated level. Equally notable, 77\% of cases (23/30) exhibited clear \textbf{folding-back} patterns, with students recursively returning from advanced conceptual layers to earlier understanding and rebuilding with greater precision.
...
\subsubsection*{Theme 4: Differential Engagement Patterns, Equivalent Depth}
While student talk percentages varied dramatically (13.3\% to 100\%), coded evidence of sophisticated reasoning (reaching Inventising) remained surprisingly consistent across engagement levels. Low-talk students reached Inventising through efficient, targeted responses rather than extended discourse, suggesting that quantity of student talk may not directly correlate with depth of in-dialogue mathematical reasoning. However, high-talk transcripts showed more frequent folding-back (average 2.3 vs. 1.6 recursive moments), indicating that extended dialogue may afford richer opportunities for observable conceptual reorganization.
...
\paragraph{High-Talk Case: P76-GX-S6 (80.9\% student talk)}
This student engaged in extended mathematical dialogue while designing a deep-sea breathing alert system using Taylor series. Facing the ``ghost term problem'' when initial expansion at $x=0$ produced vanishing terms, the student independently proposed shifting the series to $x=2$, demonstrating Property-Noticing transitioning to Formalising. 

\begin{quote}
\small\itshape
\textbf{Student:} ``Let's first tackle the 'ghost term problem'. When the function and its derivatives are zero at $x = 0$, the first few terms of the Taylor expansion will all be zero. Looking at the graph of $\sin(\pi x)$ near the origin, we know that although the function value and its first few derivatives are zero at $x = 0$, we can't just rely on the first few terms. We need to calculate more terms of the Taylor series. [...] For the 'depth dilemma', when the diver is at $x = 2$ instead of $x = 0$, the Taylor expansion centered at $x = 0$ may not be very accurate. This is where the idea of a translated Taylor series comes in handy. We should expand the function $f(x)$ around $x = 2$ instead of $x = 0$. Let $y=x - 2$, so $x=y + 2$. Then we find the Taylor series of $f(y + 2)$ with respect to $y$. By doing this, we can get a better approximation for the function value at $x=2$. [...] What do you think about my solutions?''

\textbf{AI:} ``Brilliant! You've just cracked three centuries-old puzzles---let's verify these ingenious ideas like 18th-century mathematicians with quill pens...''
\end{quote}

The student exhibited strong agency, explicitly asking ``What do you think of my solutions?'' Multiple folding-back moments occurred as the student revisited error estimation approaches, ultimately reaching Inventising by proposing computational solutions balancing accuracy with real-time constraints. The AI provided supportive historical context while allowing student-driven problem-solving.

\paragraph{Low-Talk Case: P13-G4-S5 (12.7\% student talk)}
Despite minimal verbal output (150 words total), this student reached Inventising through efficient, precise exchanges focused on satellite position prediction with non-linear drag forces. When initially proposing $F=kv$ (linear drag), the AI's direct correction (``Your proposal is linear drag, not non-linear'') prompted immediate folding-back to Image-Making, where the student reconstructed understanding and refined the function to $F_d(v,h)=\frac{C_d A \rho(h)v^2}{2}$. 

\begin{quote}
\small\itshape
\textbf{Student:} ``Ignoring $h$ violates the original problem's constraints (altitude-dependent drag). However, if we strictly isolate $v$: [...]''

\textbf{AI:} ``Correct. Now, derive the 2nd-order Taylor polynomial $T_2(t)$ for $v(t) = \frac{v_0}{1+v_0 a t}$ centered at $t = 0$. [...] Step 2: Construct $T_2(t)$: $T_2(t) = v_0 - v_0^2 a t + v_0^3 a^2 t^2$. Step 3: Why is this practical? No division: Only multiplications/additions, which are hardware-friendly. Explicit error control: The error term $R_2(t)$ decays rapidly for small $t$.''

\textbf{Student:} ``When $t$ is large, use Padé Approximation.''
\end{quote}

The student's terse contributions reflected focused conceptual engagement rather than disengagement. By dialogue's end, the student independently proposed Padé approximation for large time intervals, demonstrating Inventising-level adaptation. The AI's analytical tone matched the student's preference for ``structured, logical explanations without analogies.''

\paragraph{Mid-Range Case: P123-GX-S6 (33.7\% student talk)}
This transcript exemplified balanced collaboration. The student alternated between accepting AI scaffolding and asserting agency, particularly when exploring Taylor approximations for the normal distribution. After initially struggling to identify an appropriate real-world scenario, folding-back from Image-Making to Primitive Doing allowed reconstruction: ``It's impossible to find the area under the curve of the normal distribution.'' 
...
The AI's adaptive prompting facilitated progression to Property-Noticing, where the student recognized approximation limitations and independently proposed interval shrinking. Multiple recursive moments demonstrated iterative refinement as the student moved from calculator design to generalizing convergence properties, ultimately reaching Inventising by applying learned principles to novel engineering contexts.
...
\subsubsection*{Teaching Assistant Interview Insights}
