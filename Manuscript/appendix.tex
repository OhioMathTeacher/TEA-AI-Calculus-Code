...
\begin{tcolorbox}[colback=blue!5,colframe=blue!40,boxrule=0.5pt,arc=1mm,left=4pt,right=4pt,top=2pt,bottom=2pt]
\small\textbf{Note:} Downloadable Python code implementing this protocol is freely available at:\\ \url{https://github.com/OhioMathTeacher/TEA-Taylor-Series-Paper/blob/main/Scripts/pk_screen_v2_2.py}
\end{tcolorbox}
...
The Phase I counting pipeline is fully deterministic: re-processing the same transcript files under the same configuration yields \textbf{identical} counts. Because the speaker attribution and word counting are implemented as rule-based Python scripts (not LLM-based), there is no stochastic variation. A scripted pipeline applied the rules uniformly across all transcripts; implementation details and example input/output are provided in the Supplement (Reproducibility Note).
...
\footnotesize\textit{Note.} Cases flagged as probable AI-profiling or demonstration scripts were excluded from anchor-case selection but retained in descriptive analyses. Cases showing 0 AI words (e.g., P31, P52, P53) reflect transcripts where student-authored prompts or reflections were exported without the corresponding AI responses; cases showing 0 Student words (e.g., P06, P07, P27) represent AI-only demonstration outputs or incomplete exports. P96 shows 0/0/0 due to OCR limitations on the source document; this placeholder row was retained for dataset continuity.
\end{tablenotes}
\newpage
...
\begin{tcolorbox}[colback=blue!5,colframe=blue!40,boxrule=0.5pt,arc=1mm,left=4pt,right=4pt,top=2pt,bottom=2pt]
\small\textbf{Note:} Downloadable Python code implementing this protocol is freely available at:\\ \url{https://github.com/OhioMathTeacher/TEA-Taylor-Series-Paper/blob/main/Scripts/pkwap_analyzer.py}\\
A Flask-based transcript reviewer tool for case selection is also available at:\\ \url{https://github.com/OhioMathTeacher/TEA-Taylor-Series-Paper/blob/main/Scripts/transcript_reviewer.py}
\end{tcolorbox}
...
Transcripts with negligible student contribution (i.e., $<$10\%) or negligible AI contribution (i.e., 100\% student words) were excluded and replaced with the next-highest or next-lowest cases, respectively, to ensure sufficient dialogic material for Pirie-Kieren analysis. Finally, we reviewed the full set of memos to identify recurring themes, interpret contrasts across the high- and low-engagement groups, and generate analytic trends related to recursive reasoning, conceptual depth, and student agency.
...
\textbf{Qualitative Selection (10 noteworthy cases):} From the remaining middle-range transcripts, we identified cases exhibiting distinctive features warranting deep analysis. To systematically review this subset, we developed a web-based Flask application (shown below) that displayed original student submissions alongside categorical tags. This tool enabled rapid, consistent annotation across the following categories:
\begin{itemize}[itemsep=0.1em]
...
\midrule
1. Primitive Knowing & \ldots & \ldots \\
...
\item \textbf{FB-2 (Pages 06--07): From scenario to property.} After selecting \textbf{CT imaging} (Primitive Knowing/Image-Making), the student compresses the core instability to ``\textbf{sensitivity to measurement errors}'' (Property-Noticing). This is a concise leap from context to governing mathematical property (ill-posedness).
...
\item
Code for all 8 Pirie–Kieren layers (Primitive Knowing → Inventising).
\item
